...
class Creator {
  ...
  async downloadTemplate(choiceTemplateName) {
    ...
    const templateUrl = `tetap/${choiceTemplateName}`;
    const loading = ora("正在拉取模版...");
    loading.start();
    const result = path.join(cwd, this.projectName);
    await this.downloadGitRepo(templateUrl, result);
    const packageJson = result + "\\package.json";
    const options = {
      encoding: "utf-8",
    };
    const readPackageJsonString = fs.readFileSync(packageJson, options);
    const readPackageJson = JSON.parse(readPackageJsonString);
    console.log(readPackageJson);
    readPackageJson.name = this.projectName;
    fs.writeFileSync(packageJson, JSON.stringify(readPackageJson, null, 2));
    loading.succeed();
